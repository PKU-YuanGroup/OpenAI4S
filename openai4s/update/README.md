...
Strict discovery (`allow_single_witness=False`) always fetches fresh witnesses, including after a cached read. Cache publication uses a unique, owner-only temporary file per writer. Distribution metadata must identify the package actually loaded before it can establish a writable install channel. Both bundle probes require that the loaded package occupy the candidate root’s own `src/openai4s` layout; ancestor landmarks or symlink aliases cannot authorize updating a different tree. The interpreter’s site-packages probe likewise requires a direct package location, not an arbitrary nested copy. Git ownership beside the loaded package blocks all self-update probes, including a checkout named `src` or an editable install with matching distribution metadata.
