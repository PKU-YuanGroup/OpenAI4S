...
| `runtime/host_calls.json` | The newest 20,000 `host_call_log` rows plus per-method call and failure totals over all of them; an `args_preview` that names such a file is withheld. |
