...
- [`response_capture.py`](response_capture.py) — observes what routes really return and freezes it into [`docs/response-schemas.json`](../../docs/response-schemas.json). It wraps `make_handler`'s `_api` from the outside rather than hooking `_json`, because the gateway tests replace `handler._json` with their own collector — a hook inside the real `_json` would have missed nearly every route while appearing to work. Nothing here runs in production.
