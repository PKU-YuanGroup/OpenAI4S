...
select_install() {
  ...
  ln -s "${APP##*/}" "$POINTER"
  mv -Tf "$POINTER" "$APP_ROOT/current"
...
install)
  ...
  if [ -e "$APP" ]; then
    echo "an incomplete installation already exists at $APP; run doctor before retrying" >&2
    exit 1
  fi
  mv "$STAGED_APP" "$APP"
  select_install "$APP"
...
serve)
  ...
  exec "$APP/bin/openai4s" serve \
    --host "$HOST" --port "$PORT" --no-browser >>"$DATA_DIR/logs/app.out" 2>&1
  ;;
