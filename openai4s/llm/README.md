...
| [`transport.py`](./transport.py) | The only place in the package that opens a socket: JSON POST and SSE decoding over `urllib`, no provider SDK. HTTP and connection failures surface as `TransportError`, which is what makes a retry possible at all. The policy is deliberately narrow: only a request that committed nothing may be replayed — a whole-response POST can be, a stream that already handed events to the caller cannot — with bounded attempts, exponential backoff under full jitter so that many clients rate-limited at one instant do not all come back at one instant, `Retry-After` beating the computed delay, cancellation polled *during* the wait rather than only on either side of it, and a total budget so a five-minute `Retry-After` cannot silently park a turn. A non-empty stream event that is not valid JSON is raised rather than skipped, because a dropped event could have been a tool call; the offending chunk is truncated in the error text. |
