...
| 目录 | 职责 |
| --- | --- |
| `dist/` | [`../../../frontend/`](../../../frontend/) 提交进来的 Vite 构建产物（F-03 空壳）。`OPENAI4S_WEBUI_NEXT=1` 时 `dist/index.html` 成为 `/` 与工作台深链的 SPA 外壳；`/static/dist/` 无论开关都是本目录下的普通静态树。在 `frontend/` 里 `npm run build` 重建；不要手改带哈希的资源。脚本全部是带 `src=` 的外链，这棵树仍落在 CSP `script-src 'self'` 里。 |
