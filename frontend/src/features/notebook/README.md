...
| [`notebook.test.ts`](notebook.test.ts) | Merge, replay dedup, invalidate timings, scroll gate, traceback XSS, a streaming `<pre>` repainted in full after it was elided, a finished record shown exactly, the painted list held while reading, projections and records reused when unchanged. |
