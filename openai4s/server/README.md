...
- [`errors.py`](errors.py) — `GatewayError` and the stable machine error codes. Its own module because `GatewayError` sat ~5,800 lines below gateway's own import block, so a sibling module importing it from there is a circular import that fails the daemon at *boot*. Every route group carved out of `Handler._api` needs to raise it, so the cycle would otherwise be rediscovered once per extraction. Gateway re-exports the old names.
- [`kernel_routes.py`](kernel_routes.py) — the twelve kernel routes, moved verbatim out of `Handler._api` as the first slice of its decomposition (2,100 lines / 261 branches → 1,887 / 237). Chosen because it was the only group that could be *checked*: it owns eleven frozen response shapes where `memory`, `permissions`, `connectors` and `compute` own none. `handle()` is tri-state — True emitted a response, False means the path matched but the method arm did not fire and the chain must continue to its 404; `return bool(regex_matched)` would swallow twelve wrong-method 404s. Two position dependencies are load-bearing and tested rather than merely commented: the call site must stay after the `frame_mutation` guard (the only write-protection on the seven mutating routes, including code execution) and after the `workbench` guard (which is the entire 404 for `GET /frames/{id}/execution`).
