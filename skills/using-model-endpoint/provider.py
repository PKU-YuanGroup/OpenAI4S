...
from openai4s_compute_provider import ByocError, ExecResult
...
class InferProvider:
    ...
    # name that is safe to SET in this process. Mirrors the system-reserved
    # variables, the injection-time env vars, and the unsafe alias names —
    # keep this denylist in sync with those sources.
    _alias_name_re = re.compile(r"(?!LD_|DYLD_)[A-Z][A-Z0-9_]{0,127}")
    _alias_denylist = frozenset(
        (
            # shell/system-reserved variables:
            "PATH",
            "HOME",
            "USER",
            "SHELL",
            ...
            "GIT_SSH_COMMAND",
            "GIT_ASKPASS",
            # injection-time env vars (names not already above; LD_*/DYLD_*
            # ride the regex):
            "PYTHONHOME",
            "SHELLOPTS",
            "PS4",
            "ZDOTDIR",
            # unsafe alias names (TLS trust roots + proxy routing):
            "SSL_CERT_FILE",
            "SSL_CERT_DIR",
            "REQUESTS_CA_BUNDLE",
            "CURL_CA_BUNDLE",
    ...
    def apply_auth(self, creds: dict[str, str]) -> None:
        ...
        if (
            ...
            and self._alias_name_re.fullmatch(alias)
