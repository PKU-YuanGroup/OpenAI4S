...
| [`render.ts`](render.ts) | `renderMd` / `mdInline` / `mdCodeBlock`。F-21 把表格包进 `.md-table-wrap`。0.2.0 存下的单段 `/api/artifacts/<id>` 链接（契约 v1 网关一律 404）改写到 `/api/v1/artifacts/<id>`，其它 href 原样保留。 |
