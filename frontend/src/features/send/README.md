...
| [`permission.ts`](permission.ts) | Permission gate cards. Frozen DOM classes `.perm-card` / `.resolved` / `.allowed` / `.denied`. A 202 `decision_resolving` (accepted, commit still in flight) keeps the buttons down with a neutral status line (feature-local copy) until `permission_resolved` settles the card. |
| [`permission.test.ts`](permission.test.ts) | 202 `decision_resolving` is not a failure: no retry offered, no failure hint, and `permission_resolved` settles the card; a real refusal (410 expired) still offers the buttons again. |
...
| [`plan.test.ts`](plan.test.ts) | The terminal plan card: a completed plan with a step in progress is not shown as complete, the live glyph stays while executing, and a fully settled plan still reads complete. A draft's change request that is not dispatched (a turn is running, the POST failed) goes back into its box. A discard answered after the user opened another session leaves that session's plan card and state alone. The legacy approval card appears only after a plan-mode turn that finished: a stopped, guardian-blocked or failed one offers nothing, and leaves nothing pending for the next turn. |
...
| [`step.ts`](step.ts) | Semantic activity steps, `buildStepCard`, `searchResultHttpUrl`. Opening an artifact from a step reports a viewer failure as a hint. History and live updates build a collapsed card's body on its first expand; `buildStepCard` itself (the window contract) builds at once. |
| [`step.test.ts`](step.test.ts) | Step cards: reopening a running session, the replayed turn updates the stored card rather than adding a second; a collapsed history or updated card has no body until opened, then shows the latest step; self-opening cards and `buildStepCard` build at once; the output toggle, a running review and an untitled finding follow the UI language; an artifact the viewer fails to open is reported, not left as an unhandled rejection. |
