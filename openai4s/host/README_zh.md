...
| [`completion.py`](completion.py) | Cell 唯一的成功契约。它校验一个 `output`、1–4 条已完成 action bullet 和可选的 output schema，并为当前 dispatch context 留下一份通过校验的 submission。bullet 的英文过去时检查接受规则的 `-ed` 形式、列出的不规则过去式（`took`、`did`、`cut`、`reset`），以及在拼写不同于原形的不规则过去式前加 re-/over-/un-/mis-/out-/up-/pre- 前缀的形式（`overwrote`、`rebuilt`、`undid`）；拼写与原形相同的过去式不接受前缀，所以 `output`、`upset` 仍被拒绝，将来时和进行时也一样。这项检查不套在 CJK bullet 上——它们的形态不带时态。接入证据提供者后，核对是逐条主张进行的：`output` 里点名的每个文件或 Artifact 都必须有 Artifact 存储、某个已执行 Cell 记录的写入、或文件系统作为凭据，摘要里复述的数字也必须和同一份 submission 的 metrics 一致。大到或深到无法穷尽核对的 output 直接拒绝，而不是只查够得着的部分；反过来，证据提供者自己抛异常时则退回旧的不核对接受——把每个诚实的 submission 都拒掉，会把循环唯一认可的完成信号锁死。 |
...
| [`zenodo.py`](zenodo.py) | 只读取元数据的 Zenodo 数据集检索，走同一个 search 信封。记录 DOI 与 concept DOI 分开保存，保留精确的文件 key、声明的大小与校验和，文件 URL 由记录 ID 和 key 构造，从不跟随上游给的链接。未知的许可、大小、校验和与文件清单保持未知（受限或禁运记录的空文件列表不等于空数据集），它从不下载数据集文件。 |
