...
| `sender.py` | 整棵树里唯一发送遥测的代码，而它所有值得一提的地方都是拒绝。没有同意就不发——而且这个检查发生在**解析域名之前**，因为对 `log.openai4s.org` 的一次 DNS 查询本身就是同意所要授权的那个信号。不跟随重定向（那是第三方在决定研究遥测去哪里），不用明文 HTTP，URL 里不能带凭据，不接受非 `seal` 产出的载荷。没有能熬过撤销的队列、没有比撤销活得更久的重试、退出时不 flush。 |
| `emit.py` | 程序其余部分唯一要调的入口。它绝不抛异常（遥测的 bug 不能变成失败的 turn）、绝不阻塞（发送在 daemon 线程上，慢的 collector 不给 turn 增加任何耗时）、每次都重新读同意（没有能熬过撤销的缓存「已启用」）。在常见路径——没有同意——它就是一次读设置然后返回。`turn_outcome` 把 engine 的 stop-reason 词汇映射到已声明的 `outcome` 枚举，是一个纯的、经测试的函数，而不是在调用点上的猜测。 |
