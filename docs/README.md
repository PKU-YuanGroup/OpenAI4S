...
| `docker.md` | Bilingual container guide: the image, `compose.yaml`, the Kubernetes manifests, and what a wildcard bind actually changes. It states the trade a container makes with the kernel sandbox rather than implying the container replaces it, and lists the limitations that are real today — no R, no IPv6, no access logs, a credential-bearing startup line, and the secret-broker variable that looks like the answer for a Kubernetes Secret and silently is not. |
