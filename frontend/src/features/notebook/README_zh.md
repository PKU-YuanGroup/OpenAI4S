...
| [`kernel.ts`](kernel.ts) | `_kc` invalidate、kernel/REPL/env，供 F-11 调用的 `notebookOnTurnDone`。每个会话同时只有一个 kernel 读取在途；跨过一次 invalidate 的回答会再读一次。`kernelView` 是面板渲染 kernel 状态所用的唯一不可变值；`_kc` 只整体替换，不原地修改。 |
...
| [`dock.test.tsx`](dock.test.tsx) | 在 node 里把面板组件当函数调用：二进制输出的省略提示、同一 cell key 在完成前后保持同一组件类型、CellList 不订阅任何 cell store、状态行按最近一次 kernel 读取渲染。不叫 `Notebook.test.tsx`：在大小写不敏感的磁盘上它与 `notebook.test.ts` 同名，tsc 会漏掉其中一个。 |
...
| [`notebook.test.ts`](notebook.test.ts) | 合并、重放去重、invalidate 时机、滚动门控、traceback XSS、被省略后重新出现的流式 `<pre>` 完整重绘、已完成的记录按原文完整显示、阅读时保持已绘制的列表、未变化时复用投影与记录对象、kernel 读取按会话隔离并在 invalidate 后重读。 |
