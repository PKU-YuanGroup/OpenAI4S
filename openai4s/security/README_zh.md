...
| [`fsprobe.py`](fsprobe.py) | 基于 `os.lstat` 的 `lstat_is_symlink` / `lstat_kind`，给那些「读不到就必须拒绝」的守卫用。CPython 3.14 把 `Path.is_symlink()`、`is_dir()`、`is_file()`、`exists()` 改到了 `os.path` 之上，而后者会吞掉所有 `OSError`，于是 `try: p.is_symlink() except OSError: 拒绝` 这种守卫在容器所用的解释器上不再拒绝，而是把读不到的路径报成「不是符号链接」。这些探针对缺失给出明确答案，其余错误一律上抛；`host/files.py` 的 secret 别名遍历、环境代际布局检查、动态 scope 删除、陈旧 artifact 清理、工作区快照和 bring-up 校验器都经由它们读取。 |
...
- 三道 LLM 筛查器只计量、绝不设闸，其消耗记在 `llm_screening_*` 上，位于 `ENFORCED_QUOTA_KINDS` 之外。拒绝一次筛查并不能省下这些 token，只会让这个 Cell、或那段工具输出，在筛查关闭的状态下继续走下去。所以一个无法被拒绝的控制，自己也不该有能力拒绝别人——注入扫描的 prompt 是 16 KB 由攻击者选定的文本，把它记进可强制的配额，等于让一个精心构造的页面借着保护成员的控制去吃掉成员的配额窗口。
- 在 Web 守护进程里，必须把会话**解析后**的 LLM 配置交给这些筛查器，而不是启动时的那份。两者都读 `cfg.llm.api_key`，为空就失败即放行；而在文档所述的安装方式下它永远为空，因为密钥存在 settings 里——于是拿到启动 Config 的筛查器会报出一次它从未做过的通过。
