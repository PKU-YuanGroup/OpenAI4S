...
| [`test_r_kernel.py`](test_r_kernel.py) | **包含被重写的变量检查器契约。** `substitute()` 不会替换来自 `.GlobalEnv` 的绑定——而这正是检查器唯一读取的环境——于是每个普通变量都返回不透明的 `symbol`，没有类型、没有长度、没有预览；而原先的测试把这个结果写成了「契约」。这里把「安全」和「没用」混为一谈了：读一个普通绑定什么都不会运行，只有 promise 才有东西可求值。现在检查器先用 `substitute` 探一次，探不出名堂时才真正去读。明说而非隐瞒的代价：在当前版本的 R 上，检查一个 `delayedAssign` 绑定会强制求值；而 promise 的函数体如果抛错，会退化成那个不透明的回答，而不是让整份列表失败——这正是问题浮现的方式：一个 `stop()` 让所有变量都变成「failed closed」。调用 active binding 依然是绝对禁止的。 另外还覆盖了 R 侧一直没有的实时输出：宿主端一直接受 `stdout_chunk` 帧，而 R worker 一帧都没发过，于是一个跑得久的 R cell 在结束之前什么都看不到。流式输出发生在顶层表达式之间——求值器本来就在逐个遍历它们——而且这个限制是真实存在并被写明的：一个话痨 `for` 循环仍然是一次性到达的，因为在单线程解释器里做表达式内部的流式输出需要 C 层的工作。 |
