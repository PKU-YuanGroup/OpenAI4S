...
| [`linux_sandbox.py`](linux_sandbox.py) | 在 bubblewrap 下检查同样的四条边界。它断言 backend 确实是 bubblewrap——一次回退后仍然通过的运行，报告的是它根本没测过的边界。Ubuntu 24.04 上的独立 CI 任务（`linux-sandbox-full`）：enforce 模式，不允许 raw-network 覆盖（覆盖即为硬失败）。该 check run 绑定冻结 SHA；release workflow 的 `platform-checks` 矩阵在多次定时全绿加候选 SHA 之前仍不重跑它。详见 [`docs/platforms.md`](../../docs/platforms.md)。 |
