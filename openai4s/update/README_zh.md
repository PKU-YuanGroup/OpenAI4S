...
| [`channel.py`](channel.py) | `detect(cfg) -> Channel`，按顺序解析：容器、macOS `.app`、WSL 托管 bundle、Linux 可重定位 bundle、源码 checkout、venv、unknown。`OPENAI4S_CHANNEL` 可以直接钉住一个拒绝型渠道，对会自更新的渠道则只能确认；探针不认可的钉住值会被拒绝，拒绝文案会点名它。容器探针看四个正向信号而不是两个：containerd 和 CRI-O——也就是 Kubernetes 真正跑的东西——既不写 `/.dockerenv` 也不写 `/run/.containerenv`，而一个以 root 跑的 Pod 里 site-packages 是可写的，于是 venv 探针会命中，更新会落进一个下次重启就丢掉的镜像层。每一个分支都把命中的探针和做出判断的绝对路径记进 `Channel.evidence`；它是管理员可见的，`as_dict()` 默认不带它，除非调用方明确要。`require_self_update` 是拒绝一个被拒绝渠道的唯一地方。凡是会给出 `self_update` 的探针，只能依据关于本进程的事实——解析后的包路径、解析后正在运行的解释器、平台、解释器自身的 site 配置。代码附近的文件（地标、标记、分发元数据、`.env`）至多是佐证，只能把判断推向拒绝，永远不能推向写入：bundle 根恰好是 `<root>`，包位于 `<root>/src/openai4s`、由 `<root>/runtime/` 运行、且在 Linux 上，且 `<root>` 与 `<root>/src` 下都没有 `.git`（构建脚本用 `--exclude .git` 同步源码），而绝不是某个只是放着地标文件的祖先目录。`Channel.install_scheme` 说明哪个安装器写 `install_root`——`prefix`、`user` 或 `bundle`——这样 applier 从一个字段推导命令，而不是去解析 evidence 字符串。`config` 会从包的每一级祖先目录加载 `.env`，这正是一个未经确认的钉住值被当作植入文件的原因。 |
...
严格发现模式（`allow_single_witness=False`）始终重新获取摘要证人，即使已有普通检查的缓存。每个缓存写入者使用独立、仅所有者可读写的临时文件。`venv` 渠道需要关于本解释器的两件事：包所在目录**就是**它的 purelib 或 platlib（而不是其下某处——那里住着 pip 永远不会碰的 vendored 副本），或者在解释器不是虚拟环境且启用了 user site 时就是它的 user site；并且 pip 为 `openai4s` 写下的 `RECORD` 就在同一目录里，位于一个目录名和 `Name:` 头都是 `openai4s` 的 `*.dist-info` 中，且列出了 `openai4s/__init__.py`。刻意不参考 `site.getsitepackages()`：在 `--system-site-packages` 下它会给出基础解释器的目录，而本解释器的 pip 并不往那里写。这两个路径都来自 `sysconfig`，而它是惰性初始化的，会在 `config` 加载完 `.env` **之后**才从 `os.environ` 读取 `_PYTHON_SYSCONFIGDATA_NAME`，所以它的回答只有位于 `sys.prefix` 或 `sys.exec_prefix` 之下时才被接受——这两者在解释器启动时就已固定。前缀位于 uv 缓存中的解释器（`uvx`、`uv tool run`、`uv run --with`）会被拒绝，因为那棵树归缓存所有；在虚拟环境之外被标记为 EXTERNALLY-MANAGED 的解释器也会被拒绝，因为 pip 本来就会拒绝往里安装（PEP 668）。只是挨着包放着的元数据不算证据——setuptools 会在任何开发树里留下 `openai4s.egg-info`，所以用 `python -m` 运行一个解压出来的源码归档时，不需要任何攻击者就会带着它。元数据文件以不跟随链接、不阻塞、有字节上限的方式读取。被加载的包旁边若有 git 所有权（`.git`），任何自更新探针都会被拦下，包括名为 `src` 的 checkout，以及带着匹配分发元数据的 editable 安装。
