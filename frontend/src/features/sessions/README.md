...
| [`chrome.test.ts`](chrome.test.ts) | Hint error prefix (`错误：` / `Error: `) without a new i18n key; `reportFailure` shows a failed fire-and-forget action as an error hint; `watchActivateKeys` observes the dock tabs, the Files pane and the transcript's tile strips instead of the whole body. |
| [`chrome.ts`](chrome.ts) | `hint`, `reportFailure` (the rejection handler of fire-and-forget actions), disconnect banner, `openMenu` Esc/`role=menu`, keyboard activate (scoped to the hosts that receive activatable nodes). |
...
| [`dashboard.ts`](dashboard.ts) | Home list, project search / load-more / retry, example CTA poll bound to view lifecycle, and the dashboard's one 4s poll, which other features ride through `onDashPoll`. |
| [`dashboard.sessions.test.ts`](dashboard.sessions.test.ts) | The dashboard's poll drives the Running card and every rider from one interval, reading at once when it starts. The dashboard's session lists and the example CTA: a failed `/frames` read says so with Retry, keeping the rows last read, rather than showing no sessions and offering the example; the CTA's status poll runs once however many repaints raced its first read, and never after the dashboard was left. |
...
| [`load.render.test.ts`](load.render.test.ts) | A sessions read rebuilds the sidebar twice (its loading state, then the settled lists), not a third time from the folder read it drives. |
