...
def test_external_mutation_admission_is_noop_while_stage1_is_disabled(
    tmp_path,
    monkeypatch,
):
    ...
    state.turn_lock.acquire()
    try:
        result = runner.upload_artifact(
            {
                "frame_id": frame_id,
                "filename": "legacy.txt",
                "content_text": "legacy behavior",
            }
        )
        assert result["artifact_id"] == "legacy-result"
        assert len(calls) == 1
    finally:
        state.turn_lock.release()
        runner.close()
...
def test_frameless_project_mutation_barrier_is_noop_while_stage1_is_disabled(
    tmp_path,
    monkeypatch,
):
    runner = gateway_mod.SessionRunner(_cfg(tmp_path, stage1=False), _Hub())
    ...
    try:
        ...
        assert result["artifact_id"] == "legacy-project"
        assert len(calls) == 1
    finally:
        with runner._lock:
...
def test_flag_off_project_delete_preserves_legacy_frameless_file(tmp_path):
    runner = gateway_mod.SessionRunner(_cfg(tmp_path, stage1=False), _Hub())
    ...
    try:
        runner.delete_project(project_id)
        assert runner.store.get_artifact(saved["artifact_id"]) is None
        assert path.read_text() == "legacy retained bytes"
    finally:
        runner.close()
