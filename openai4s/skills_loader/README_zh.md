...
| [`bundled_network_inventory.json`](./bundled_network_inventory.json) | 给字节精确 pinned 的第三方集合准备的 loader 侧网络 mode 清单。`skills/bioskills/` 不能改文件本体，因此成员在这里标成 `legacy`/`unknown`。这份清单永不授权 egress。 |
| [`capabilities.py`](./capabilities.py) | 闭集 `capabilities.network` schema（`none` / `host_only` / `raw_required`）、canonical digest、frontmatter 解析，以及离线 catalog readiness。manifest 只声明需求、永不授权。内置 `raw_required` 一律 fail closed。catalog 只做本地判定（`checked_locally: true`，`probed: false`），零 socket、零 subprocess。 |
