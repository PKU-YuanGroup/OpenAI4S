...
| [`scroll.ts`](scroll.ts) | 跟随 + `_nbReading` / `_nbDirty` / `_nbSched`（app.js:10339-10350, 9900-9908）。`_nbReading` 只跟随 Notebook 自己的滚动：dock body 由所有面板共用。 |
...
| [`notebook.test.ts`](notebook.test.ts) | 合并、重放去重、invalidate 时机、滚动门控、traceback XSS、被省略后重新出现的流式 `<pre>` 完整重绘、已完成的记录按原文完整显示、阅读时保持已绘制的列表（滚动其他面板不算）、未变化时复用投影与记录对象、kernel 读取按会话隔离并在 invalidate 后重读、Notebook 隐藏时不读取 kernel、REPL 提交与 fork 请求同一时间各只有一个、复制只在确认写入后才报成功。 |
