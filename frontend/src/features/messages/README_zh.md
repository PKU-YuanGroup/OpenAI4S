...
| [`stream.ts`](stream.ts) | `feed` / `flushRender` / `scheduleRender` / `startStream` / `sealText`。已稳定的文字只渲染一次，按块追加到 sealed 节点；每次只重渲染尾部（仍可能被后续列表项延续的列表留在尾部）；无法稳定的尾部（未闭合的代码块）越长，两次刷新的间隔越大，每秒的重渲染工作量保持不变。`startStream` 保留仍在页面上的步骤卡片登记，重开运行中的会话时，重放的回合会更新已存的卡片，而不是再添一张。 |
| [`stream.test.ts`](stream.test.ts) | 流式回答显示的内容与整段渲染完全一致，同时每个已稳定的段落只渲染一次（线性工作量）；松散列表不会被拆开；不断增长的未闭合代码块每秒只刷新几次而不是二十次；最终渲染是整段文字；被替换的回答在新节点里重新开始。 |
