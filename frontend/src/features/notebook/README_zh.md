...
| [`cells.ts`](cells.ts) | 合并、draft/start/chunk/finished、`_seenChunks`、每 cell 输出 signal、`loadExecutionLog`、`paintStreamedText`（新的 `<pre>` 从零开始）、`notebookViewEntries`（阅读门控下一次渲染可以绘制的列表）。 |
...
| [`notebook.test.ts`](notebook.test.ts) | 合并、重放去重、invalidate 时机、滚动门控、traceback XSS、被省略后重新出现的流式 `<pre>` 完整重绘、已完成的记录按原文完整显示、阅读时保持已绘制的列表。 |
