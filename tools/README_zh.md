...
另一条渠道分发——npm，或直接从本仓库运行 `npx`——并刻意不写进
...
| 目录 | 职责 |
| --- | --- |
| [`skills-installer/`](skills-installer/) | `openai4s-skills` 命令——`npx @pku-yuangroup/openai4s-skills@0.2.0 <command>` 固定使用 npm 发布版（603 个 Skill：42 个精选 + 561 个 bioSkills），`npx @pku-yuangroup/openai4s-skills <command>` 使用 npm 最新发布版，`npx github:PKU-YuanGroup/OpenAI4S <command>` 使用当前仓库目录（604：43 + 561）。它把所选的 Skill 库复制到 Claude Code、OpenAI4S 数据目录，或用户指定的任意目录——并写下清单，使卸载精确、覆盖默认拒绝。 |

## 它处在什么位置

仓库根目录的 `package.json` 声明了这个 npm 包：`bin` 指向
`skills-installer/cli.mjs`，`files` 列表把该目录连同 `skills/` 一起发布。把清单
放在根目录，使 `npx github:PKU-YuanGroup/OpenAI4S` 可以直接使用默认分支，
独立于 npm 上的发布版本。
