...
| `runtime/frames.json` | The frame tree, delegated children with their status and stop reason, each child's own Action Ledger (rooted at the child frame, so never in `ledger.json`), child kernel generations, and which frame ran each Notebook Cell (`notebook.json` stays frame-free). A child's `error`, stored as `str(exc)`, travels as a kind, a length and a fingerprint; its model-endpoint override as endpoint facts; a free-text stop reason as `text`. |
| `runtime/activity.json` | Every activity card of the root and its children with `created_at` and `updated_at`, not only review cards. A card that names a file the artifact filter refuses by name (`.env`, `credentials.json`, `*.pem` …) keeps its kind, title, status and times, with input and output withheld; so does such a child-ledger entry. |
| `runtime/host_calls.json` | The newest 20,000 `host_call_log` rows plus per-method call and failure totals over all of them. An `args_preview` is withheld when it or the row's resource keys (which hold the path in full) name such a file, and when a `write_file`/`edit_file` preview was truncated: argument order is the caller's, so the 500-character cut can drop the path and keep the body. |
...
| `runtime/compute_jobs.json` | Remote compute jobs the session's workspace submitted (a job's only link to a session): status, exit code, the closed `termination_reason`, event kinds and times; the free-text `reason` as a kind, a length and a fingerprint. The host alias is fingerprinted; remote paths, process ids, receipts and event payloads stay behind. |
