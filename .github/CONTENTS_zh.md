...
| `dependabot.yml` | 每周一把 `uv`、`npm`、`docker`、`pre-commit` 和 `github-actions` 的版本更新合并为一个 PR，包括大版本、生产依赖和格式化 hook。npm 条目同时覆盖根目录的浏览器工具和独立的 `frontend/` 应用。通配符将所有符合更新条件的依赖纳入分组，不设置 allow/ignore 过滤。安全更新独立于每周版本更新分组，遵循 GitHub 的安全更新设置。 |
