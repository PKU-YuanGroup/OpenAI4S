...
| [`renderers.test.ts`](renderers.test.ts) | Text renderer: one read per render (no download/text cycle); sequences render as source; a late descriptor for a replaced body runs no glue. |
