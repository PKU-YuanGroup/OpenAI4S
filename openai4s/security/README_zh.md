...
| [`sandbox.py`](sandbox.py) | 用 Seatbelt（macOS）或 bubblewrap（Linux）包装 worker 命令。它构造工作区、私有临时目录、secret 读取屏蔽和网络策略，用一次真实的 allow/deny 自测来验证策略确实生效，并回报一份状态，其中的 `state` 是 `enabled`、`disabled` 或 `unavailable`。探测与命令执行都可注入，因此即使身处一个不允许嵌套沙箱的父沙箱里，这些受支持的路径仍然测得了。 |
| [`secret_broker.py`](secret_broker.py) | 把凭据放在一个不透明引用后面，业务表里存的是指针而不是 secret。只用标准库，所以系统存储都通过各自的 CLI 驱动——macOS 上是 `security`，Linux 桌面上是 `secret-tool`——而且一律走 stdin，因为命令行上的值在整个调用期间对本机任意 `ps` 都是可读的。第三个后端是环境注入，也是服务器真正会用的那个：凭据来自 unit 文件或 Kubernetes Secret，一个字节都不落盘；而且它是只读的，应用不能背着运维把运维掌管的值覆盖掉。`OPENAI4S_SECRET_STORE` 沿用沙箱那套词表，其中 `auto` 是 **fail closed** 而不是降级——它过去会带着一句启动 warning 退回明文，结果最没能力保护 secret 的那种部署，恰恰就是悄无声息什么保护都没有的那一种。这里有意不提供混淆文件后端：把密钥和密文放在一起的所谓加密，只是换一套听起来像那么回事的词来描述一个明文存储。 |
