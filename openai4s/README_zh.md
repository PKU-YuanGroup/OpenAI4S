...
| [`cua.py`](./cua.py) | 受管的 CUA 云电脑 connector：固定且公开的 transport 元数据、本地应答的固定六工具描述符面、只经 SecretBroker 解析的独立 CUA API Key（出站前才组装 `Authorization: Bearer` 头）、带内鉴权失败判定、凭据脱敏，以及网页与 Agent 共用的固定形状 ping/observe 投影。 |
