...
| **Chemistry / domain analysis** | `admet_genetic` · `retrosynthesis_planning` · `mineral_spectra_analysis` |
| **Research workflow** | `literature-review` · `pdf-explore` · `paper-narrative` · `figure-composer` · `figure-style` · `indication-dossier` |
| **Analysis / reproducibility** | `example_stats` · `audit-dataset` · `evaluate-model` · `plan-ml-experiment` |
| **Platform** | `remote-compute-nvidia` · `remote-compute-ssh` · `using-model-endpoint` |

The four analysis/reproducibility Skills use pure-stdlib sidecars and run in the
local persistent Python kernel without a separate environment.
