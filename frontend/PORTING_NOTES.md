...
| `applyStaticI18n` (161-167) | `runtime.ts` `applyStaticI18n` | `data-i18n` / `-title` / `-ph` / `-val`. A node with no translation yet keeps its markup text (app.js's dictionaries were synchronous; these are `import()` chunks the Shell usually mounts before). The first dictionary load re-runs the same repaint as `setLang`: static labels, `.lang-btn`, language hooks. Views rendered from data through `t()` are not repainted, so `bindWorkbench` routes to the first view only once the dictionaries have loaded (bounded by `I18N_ROUTE_WAIT_MS`). A node code writes (`#conv-title` via `setTitle`, `#proj-current` via `renderProjMenu`) drops its static attribute when it does: app.js's `rerenderI18n` put the title back after every repaint, and without that the repaint wrote "Session" into the title input, whose blur commit renamed the session. |
