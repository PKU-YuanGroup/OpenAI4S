...
| [`model_profiles.py`](model_profiles.py) | A model-provider profile passes through here on the way in, where it is validated and migrated, and again when it is persisted, activated, or removed. Credentials are stripped out of anything that becomes public. |
