...
def panel_task(outline, letter, fig_label="Figure", rules_ref="(load `figure-style`)"):
    ...
    neighbours = ", ".join(
        f"{q['letter']}={q['role']}:{q['chart_family']}"
        for q in outline["panels"]
        if q["letter"] != letter
    )
    data_line = (
        f"**Data:** `{{{{artifact:{p['data_vid']}}}}}` — {p.get('data_desc','')}"
        if p.get("data_vid")
    ...
    share_line = (
        (
            f"- **Row-mates: {','.join(rowmates)}** — match y-limits if same metric; series identity "
            f"labelled ONCE on the row (rightmost panel)."
        )
        if rowmates
        else ""
    )
...
- **Floor (section 2.1, non-negotiable):** every distinct mark, series, glyph, comparator
...
- **section 9 Render-then-verify:** after savefig, (a) `from PIL import Image; assert
...
def compose_crops(outline, dpi=300, gutter_mm=4, pad_px=4):
    ...
    :func:`compose_figure` for the section 3.5 perceptual self-QA pass."""
    W, ncol, colw, rowh, row_y, g = grid_geom(outline, dpi, gutter_mm)
    H = row_y[-1] + rowh[-1]
    out = {}
...
def review_schema(per_panel=True):
    ...
    if per_panel:
        v_props["panel_letter"] = {"type": "string"}
    return {
        "type": "object",
        "properties": {
            "editor_verdict": {
            ...
            "outline_revisions": {
                "type": "array",
                "description": "Figure-level changes that no single panel can fix in isolation: grid geometry "
                "(rowspan/colspan/row_heights), panel add/remove/merge, row-header vs per-panel "
                "titles, label_budget reallocation, whitespace fill.",
                "items": {
                    "type": "object",
                    "properties": {
                        "kind": {
...
def composite_review_task(
    composite_vid, outline, rules_vid, prev_vid=None, round_no=1, min_floor=5
):
...
1. **Outline level** (`outline_revisions`): the layout, grid, panel set, title strategy.
   - section 3.5 Fill the box: any panel with >25% dead whitespace, or whose natural aspect doesn't
     fit its slot → propose rowspan/colspan/row_heights change.
   - section 2.4 Titles: any title that fails the "read it aloud cold" test (cryptic noun fragments),
...
def derive_outline(figure_png_path, claim=None, data_hints=None, model=None):
    ...
    prompt = (
        "Reverse-engineer this multi-panel figure into a figure_outline. "
        ...
        + (
            ...
            if claim
            else "Infer the figure's one-sentence claim from its title and panel a.\n"
        )
        + (f"Data hints: {data_hints}\n" if data_hints else "")
    )
    r = fc_sdk().llm(
        prompt,
        images=[figure_png_path],
