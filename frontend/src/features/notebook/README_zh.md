...
| [`notebook.test.ts`](notebook.test.ts) | 合并、重放去重、invalidate 时机、滚动门控、traceback XSS、被省略后重新出现的流式 `<pre>` 完整重绘、已完成的记录按原文完整显示、阅读时保持已绘制的列表、未变化时复用投影与记录对象、kernel 读取按会话隔离并在 invalidate 后重读、Notebook 隐藏时不读取 kernel、REPL 提交与 fork 请求同一时间各只有一个。 |
