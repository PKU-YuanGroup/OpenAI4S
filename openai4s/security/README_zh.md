...
| [`sandbox.py`](sandbox.py) | 用 Seatbelt（macOS）或 bubblewrap（Linux）包装 worker 命令。它构造工作区、私有临时目录、secret 读取屏蔽和网络策略，用一次真实的 allow/deny 自测来验证策略确实生效，并回报一份状态，其中的 `state` 是 `enabled`、`disabled` 或 `unavailable`。团队策略还会遮住 OpenAI4S 数据、其他成员的 data-root 个人区和系统临时目录中的旧 kernel，只精确开放本会话输入，并拒绝工作区中无法完全归属的既存硬链接。探测与命令执行都可注入，因此即使身处一个不允许嵌套沙箱的父沙箱里，这些受支持的路径仍然测得了。 |
