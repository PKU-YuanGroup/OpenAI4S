...
F-20 工作台外壳：团队面、模态焦点陷阱、⌘K palette、上传 / 麦克风、布局密度、列宽拖拽。团队模态走 `openModalEl` / `closeModalEl`（旧 IIFE 绕过了陷阱）。Palette 的 Artifact 命中按 M-03（先开会话，再 exact `version_id`）。
...
| [`chrome.css`](chrome.css) | 本车道样式：palette / team / resizer 类名。 |
