...
`npx @pku-yuangroup/openai4s-skills install boltz --target claude` installs the
published copy. The npm catalog can differ from this repository.
...
which is also the route to take on Windows: extract it and copy `skills/boltz/`
out (`unzip main.zip 'OpenAI4S-main/skills/boltz/*'` unpacks only that
