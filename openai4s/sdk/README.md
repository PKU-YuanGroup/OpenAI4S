...
| [`judgment.py`](judgment.py) | Worker-side `host.judge(template, state, **params)` signature. It sends a host_call and refuses a raw questions list so cell code cannot bypass the template registry. |
