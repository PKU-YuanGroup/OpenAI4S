...
| `codeql-config.yml` | 只排除一份精确、字节级固定、仅被解析而不会执行的 arXiv 测试 fixture。即使某个合成 bioSkills 示例需按单告警消除误报，可执行 bioSkills 仍保持在扫描范围内。在仓库属性 `github-codeql-config-file` 指向它之前不生效，而该属性**目前并未设置**——本仓库用的是 CodeQL 默认扫描，因此被排除的告警仍然是 open，任何离线测试也看不出这一点。启用它的命令写在文件里。离线治理测试禁止通配符，并要求被排除的路径真实存在且不可执行。 |
