...
| [`orchestration/`](./orchestration/) | 十二个场景，驱动**真的** `Reconciler` 去对一个脚本化的 backend（M4-4）。与 baseline 家族不同，它们是**故意**导入生产代码的——reconciler 的决策函数没有哪条活边界需要替身去顶，它的输入就是一行 workload 和一个 observation，两者都是数据。其中一半声明的是拒绝，而声明拒绝的场景在运行成功时判负。 |
