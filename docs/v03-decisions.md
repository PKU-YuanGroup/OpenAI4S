...
| bubblewrap PID-namespace isolation | The sandbox keeps the host PID namespace on purpose — `Kernel.interrupt()` targets `Popen.pid` exactly, and `--unshare-pid` makes bwrap interpose an init/reaper that weakens that contract. The credential-bearing half is closed (`/proc/<daemon>/environ` is bound over with /dev/null, after the `--proc` mount so it is not replaced), and the flag ordering is tested by forcing the Linux branch. What remains open: a Linux cell can still see that other processes exist. Closing that needs `--unshare-pid` + `--info-fd` child-pid parsing and a Linux run, because it changes the interrupt contract on a platform this is not developed on. The mask itself is also unverified at runtime here for the same reason. |
