...
| `GET /experimental/judgment` | Experimental semantic-judgment settings. Returns `{experimental,effective,provider,model,key_configured,disclosure,egress}`. `key_configured` is a boolean; the TypeSafe key is never echoed. `egress` reports `{mode,domain_allowed,remediation}` using `domain_in_allowlist("api.typesafe.ai")` — it does not add an egress group. `remediation` is null in `off` mode or when the host is already authorized. |
| `PUT|PATCH /experimental/judgment` | Body may include `enabled`, `capabilities:{name:bool}`, `acknowledge:{version,capabilities}`, `api_key`, `clear_api_key`. The key is stored as secret `typesafe_api_key` / scope `judgment` and is not written to `os.environ`. `acknowledge.version` must equal the current `DISCLOSURE_VERSION` or the write is 400 `disclosure_version_mismatch`. Unknown fields and type errors are 400. Returns the same shape as GET. |
