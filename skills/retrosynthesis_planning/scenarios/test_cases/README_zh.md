...
2026-09-23 数据集导入验收仅通过原生 Zenodo 导入器获取了 **`n1-targets.txt`**：
实际 465,689 字节，与上游 MD5 一致，实测 SHA256 为
`c0d1b48379e1ceb1129fba4bf3773f73f27bdb22bb4d468417e6e404d3210c15`。
显式启用的 `tests/test_dataset_import_live.py` 固定该字节身份，并检查 Store
重开后不可变产物来源。这只是单文件完整性回执；其余文件、划分、stock 规范化、
署名与预算仍需后续审查，`database_sources.json` 保持 `not_frozen`。
