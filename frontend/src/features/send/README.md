...
| [`plan.test.ts`](plan.test.ts) | The terminal plan card: a completed plan with a step in progress is not shown as complete, the live glyph stays while executing, and a fully settled plan still reads complete. A draft's change request that is not dispatched (a turn is running, the POST failed) goes back into its box. The legacy approval card appears only after a plan-mode turn that finished: a stopped, guardian-blocked or failed one offers nothing, and leaves nothing pending for the next turn. |
