...
| [`render.ts`](render.ts) | `renderMd` / `mdInline` / `mdCodeBlock`（复制按钮文字与提示走 `t()`）。F-21 把表格包进 `.md-table-wrap`。0.2.0 存下的单段 `/api/artifacts/<id>` 链接（契约 v1 网关一律 404）改写到 `/api/v1/artifacts/<id>`，其它 href 原样保留。 |
| [`render.test.ts`](render.test.ts) | `tests/browser_smoke.mjs` 的 5 个 XSS 样本；scheme 白名单；旧产物链接改写；表格容器。 |
