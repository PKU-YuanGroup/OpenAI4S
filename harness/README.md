...
`auto_mode_contract.py` is also production-independent. It is a Stage 0
contract adapter for the proposed Auto Mode user states, not evidence that a
production state machine exists. Its traces say this explicitly and pin the
order and fail-closed meaning that later stages must implement. Canonical
...
`production_state_machine: false`; later stages must supply the production
implementation.
