...
| `response-schemas.json` | The shape of every HTTP response the offline suite exercises, captured from real responses rather than written by hand. Generated by [`scripts/capture_response_schemas.py`](../scripts/capture_response_schemas.py); a diff here means a route changed what it returns. Coverage is partial and deliberately visible: a route absent from the file is one no offline test reaches. Subtrees that describe the host rather than the API — currently the kernel's `sandbox` block, whose field *types* differ between a machine that can enforce a sandbox and one that cannot — are recorded as `machine_state` instead of being pinned. Tests marked `stubbed_backend` contribute nothing: a route driven against a stubbed service returns a fabrication, and publishing that as the contract is worse than leaving the route unpinned, because a reader believes it. Those routes are watched by unit tests instead. |
| `response-contract.json` | The declared shape of every external response, hand-written rather than captured: what a route *promises*, so a captured schema that drifts from it is a finding rather than the new truth. `scripts/capture_response_contract.py` regenerates the captured side; this file is the side a human signs. |
