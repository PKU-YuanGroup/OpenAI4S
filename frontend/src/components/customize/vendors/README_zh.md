...
| [`volcengine.tsx`](volcengine.tsx) | 火山 SSO / 套餐 / key 轮询面板。await 之后写入的状态都基于写入时的最新状态更新，不再展开更早的副本。同一时间只发起一次登录；为登录打开的标签页在没有拿到授权地址时会被关闭。 |
| [`volcengine.test.tsx`](volcengine.test.tsx) | key 等待超时后，面板保留最后一次复查读到的状态，而不是等待开始时的旧状态。连按两次「连接」只打开一个登录标签页；守护进程没有返回授权地址时关闭该页，不留空白页。 |
