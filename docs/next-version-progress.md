...
| A.5 | Notebook export split menu. `notebook/export` has always accepted `python`, `r` and `bundle`; the client hardcoded `?language=bundle`, so two working formats were unreachable — wanting the Python notebook meant downloading a zip and unpacking it, and nothing said the others existed. The default action is unchanged (one click, same file) with the other two behind a chevron | *pending* | `Completed` | Removing either format fails by name; reordering so the bundle is not first fails the default-action assertion. Driven in a real browser: menu toggles, both new hrefs present |
| A.2, A.4, A.7 | Profile readiness card + probe + Gemini entry; image attachments with budgets; retrieval-source panel | — | `Not started` | — |
