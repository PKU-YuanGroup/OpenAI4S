...
| [`mcp_client.py`](./mcp_client.py) | 纯标准库的 MCP stdio JSON-RPC 客户端，外加进程级的连接 manager：每个 connector 保持一条连接，覆盖工具、资源和 prompt。服务器发起的 sampling 不在范围内。每个请求都带绝对截止时间——取值会被夹到合法区间，在调用时读取，可用 `OPENAI4S_MCP_DEADLINE_S` 覆盖——因为一个收下请求却永不作答的 connector 会把调用方无限期扣住。连接建立按 connector id 单飞，而不是压在一把全局锁下；出故障的连接会被逐出。于是一个卡死或已死的 connector，不再拖着其余所有 connector 一起等到 daemon 重启。 |
