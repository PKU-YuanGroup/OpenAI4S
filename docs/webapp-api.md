...
| `POST /frames/{fid}/message` | Starts a turn. Body `{request}` (or `{input_data:{request}}`), optional `model`, `plan`, `explore`, `task_mode`, `annotation_ids`. `task_mode` is one of `analysis_run` (default) / `reusable_pipeline` / `codebase_change`; omitted, the mode is classified conservatively from the request text, and an unrecognised value is `400 invalid_task_mode`. A non-default mode appends a guidance fragment to the turn's user message (the stored message row is unchanged) and makes the Host require and verify `source_files` / `entry_points` / `architecture_summary` / `test_evidence` on that turn's completion. With `wait:false` → `202 {"status":"accepted","frame_id","job_id","execution_id","owner":{"kind","id"},"queue_position","request_id"}`; `request_id` is the local id this turn will be named by everywhere else — the failure `frame_update`, the job result, the persisted assistant message and `GET /frames/{fid}/messages` all carry the same one, and a `wait:false` client has no other synchronous chance to learn it; default (`wait` omitted/true) blocks for the turn result.
