...
| [`states.py`](states.py) | The job vocabulary and its transition table, enforced when a status is written. Terminal states cannot be re-opened by a late probe, and `unknown` is deliberately *live*: the remote operation may or may not have landed, so it is rehydrated and reconciled rather than forgotten. |
