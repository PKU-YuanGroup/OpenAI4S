...
| [`attempts.py`](./attempts.py) | 持久化 Cell 执行 attempt 的终态判定，由 CLI 的 `LocalActionExecutor` 和 Web 的 `CellExecutionService` 共用，两处写入方因此不会再各自漂移。worker 返回了结果时，终态是 `interrupted`、`failed` 或 `completed`；Host 侧抛出异常时，watchdog 的取消是 `cancelled`，watchdog 的 `TimeoutError` 是 `timed_out`，其余交给调用方自己的兜底值。它从不读错误文本：worker 的 `error` 是用户代码的整段 traceback，某一帧里传了 `timeout=timeout`、一个名为 `timeout` 的变量引发的 `NameError`、一次 urllib 连接错误，过去都会被记成超时——Action Timeline 随后把本该是失败的 Cell 显示成被中断。 |
