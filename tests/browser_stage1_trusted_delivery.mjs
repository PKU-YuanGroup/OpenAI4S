...
async function selfTest() {
  ...
  try {
    ...
    assertion(JSON.stringify(prepared.required_package_counts) === JSON.stringify({ python: 35, r: 8 }), "fixture dependency counts changed");
...
async function runAcceptance() {
  ...
  try {
    ...
    const envRoot = path.join(owned.dataDir, "fixture-envs");
    const prepared = runFixture(["prepare", "--env-root", envRoot]);
    assertion(prepared.required_package_counts?.python === 35 && prepared.required_package_counts?.r === 8, "standard metadata fixture is incomplete");
