...
| [`render.ts`](render.ts) | `renderMd` / `mdInline` / `mdCodeBlock` (its Copy label and title through `t()`). F-21 wraps tables in `.md-table-wrap`. A one-segment `/api/artifacts/<id>` link stored by 0.2.0 (a 404 on every contract-v1 gateway) is rewritten to `/api/v1/artifacts/<id>`; every other href is left as written. Blockquote and list nesting stop at 32 levels (deeper renders flat) so no answer can overflow the stack; bracket text excludes `[`, so a long run of `[` is linear. |
| [`render.test.ts`](render.test.ts) | Five XSS samples from `tests/browser_smoke.mjs`; scheme whitelist; legacy Artifact-link rewrite; table wrap; 12,000 nested quotes or list levels render without overflowing the stack; a 60,000-`[` run is linear; an unbalanced `[` stays text. |
