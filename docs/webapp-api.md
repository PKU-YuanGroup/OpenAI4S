...
| `POST /frames/{fid}/plan/approve` | `202 {"status":"accepted","frame_id","job_id","request_id","execution_id"}` — auto-execution runs in the background; `request_id` correlates with the failure surfaces exactly as the message route's does. The plan spawner holds no coordinator ticket, so `execution_id` is a synthetic `plan-<job_id>` until the turn reaches a real execution; a failure before that point is reported under the synthetic one, and every surface after it uses the real one. |
