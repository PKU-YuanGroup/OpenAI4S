...
| [`list.ts`](list.ts) | `renderStored`, `insertMessageByTime`, framed batch paint. A plan-mode user row renders as the user's own text and a plan execution seed as a plan marker (`planPrompt.ts`). |
