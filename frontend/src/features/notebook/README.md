...
| [`notebook.test.ts`](notebook.test.ts) | Merge, replay dedup, invalidate timings, scroll gate, traceback XSS, a streaming `<pre>` repainted in full after it was elided, a finished record shown exactly, the painted list held while reading (and not after scrolling another pane), projections and records reused when unchanged, kernel reads per session and across an invalidation, no kernel reads while the Notebook is hidden, one REPL submission and one fork request at a time, Copy reporting only a confirmed write. |
