...
| [`plan.test.ts`](plan.test.ts) | The terminal plan card: a completed plan with a step in progress is not shown as complete, the live glyph stays while executing, and a fully settled plan still reads complete. The legacy approval card appears only after a plan-mode turn that finished: a stopped, guardian-blocked or failed one offers nothing, and leaves nothing pending for the next turn. |
